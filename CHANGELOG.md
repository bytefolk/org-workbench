...
- #89: `POST /hire` 与桌面 IPC 网关新增可选 `mcp` 字段（`{tools, servers}`，v0 契约加法，省略即维持今天的空 `policy.mcpTools` 行为），授予后骨架额外生成 `mcp.json`（`employee-mcp.v1alpha1`）并把 `entrypoints.mcp` 指向 `./mcp.json`，与 `employee.json` 一同计入 `packageRef.digest`；控制面逐条镜像上游 `employee-mcp.ts` 规则（小写标识符、https-only 且不含凭据/片段的 URL、大写下划线环境变量名、条数与去重上限），`mode:"read_only"` 请求 `write` 工具直接 400 拒绝而非静默降级。凭据只以环境变量「名」流转，密钥值不进入请求、不写入岗位包。创建员工抽屉新增可折叠的「授予 MCP 工具（高级）」区域。
...
- Fix for #89: `tsc -b`、`typecheck:renderer` 全绿。`node --test apps/server/dist/test/hire.test.js` 12/12 通过（含新增 3 例：省略 `mcp` 时 `mcpTools` 为空且不写 `entrypoints.mcp`/`mcp.json`；授予后 `mcp.json` 为合法 `employee-mcp.v1alpha1` 且 stdio 与 https 两种 transport 逐字段落盘；`read_only` + `write` 工具在请求闸门 400 拒绝且零文件系统副作用），边界矩阵新增 13 例非法 MCP 形状（缺一侧、未知字段、非法工具名/重复名、非法 `requestedMode`、未知 transport、缺命令、小写环境变量名、明文 http、URL 带凭据、请求头内联明文密钥、重复 server 名）。全量 `apps/server` 套件 127/145 通过、17 失败，`git stash` A/B 对比确认失败测试名集合与未改动基线**完全一致**（`diff` 无输出），系既有 test 隔离问题非本 PR 引入。`test:desktop-main` 34/34 全绿（含新增 IPC 网关正反用例）。`test:renderer` 137/148 通过，新增 `mcp-grant.test.ts` 10/10、`hire-flow.test.ts` 8/8（含「未授予时载荷不含 `mcp` 键」的向后兼容断言——`App.test.tsx` 内原有 hire 载荷断言落在既有 11 个失败里，故另在通过的套件中补该断言）；11 个失败与 #87 同一集合、同一根因（本地 `design-system`/应用两份 React 副本冲突），与本 PR 无关。`test:ui`/`security:check` 未在本地重跑——本次改动不触达那两块，交由 CI 兜底。
