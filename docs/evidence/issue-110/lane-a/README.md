# Issue #110 R2 — Lane A staging foundation evidence

## Scope and provenance

- Canonical issue: <https://github.com/bytefolk/org-workbench/issues/110>
- Consumed revision: R2 merged supersession record for #106 / #109; decision record remains <https://github.com/bytefolk/org-workbench/issues/110#issuecomment-5492495629>
...
| REQ-001 | AC-002 | `apps/desktop/packaging/runtime-layout.cjs`, `scripts/verify-packaged-app.mjs`, `scripts/test/package-layout.test.mjs`, `scripts/test/package-safety.test.mjs` | VAL-A-002-MAC: `npm run verify:package:macos`. VAL-A-002-WIN: `npm run verify:package:windows`. Focused contract: `node --test scripts/test/package-layout.test.mjs scripts/test/package-safety.test.mjs`. | Every runtime entry is present at its exact packaged path and exact bytes; missing, extra, forbidden, linked, special, or tampered content fails. | Current-main macOS verification checked 34 required entries and all 189 packaged files, including #111's PATH/environment runtime consumers and #119's exact `stable-read.js` path/bytes. The explicit missing-file production mutant fails closed. Windows verifier/test code exists but has not run natively. | AC-002: macOS exact inventory **PASS**; Windows **NOT VERIFIED**. |
