...
| REQ-001 | AC-002 | `apps/desktop/packaging/runtime-layout.cjs`, `scripts/verify-packaged-app.mjs`, `scripts/test/package-layout.test.mjs`, `scripts/test/package-safety.test.mjs` | VAL-A-002-MAC: `npm run verify:package:macos`. VAL-A-002-WIN: `npm run verify:package:windows`. Focused contract: `node --test scripts/test/package-layout.test.mjs scripts/test/package-safety.test.mjs`. | Every runtime entry is present at its exact packaged path and exact bytes; missing, extra, forbidden, linked, special, or tampered content fails. | macOS verification checked 30 required entries and all 185 packaged files, including exact source/build byte comparisons. Windows verifier/test code exists but has not run natively. | AC-002: macOS exact inventory **PASS**; Windows **NOT VERIFIED**. |
...
| REQ-004 | no dedicated AC in R3 | `apps/desktop/src/main.js`, `apps/desktop/src/packaged-smoke.cjs`, `apps/desktop/src/control-plane-launch.cjs`, `apps/desktop/renderer/src/main.tsx`, `apps/desktop/test/packaged-smoke.test.cjs`, `apps/desktop/test/control-plane-launch.test.cjs`, `apps/desktop/renderer/test/packaged-smoke-entry.test.tsx` | VAL-A-004-BOUNDARY: `node --test apps/desktop/test/control-plane-launch.test.cjs apps/desktop/test/packaged-smoke.test.cjs`; `npm run test:renderer -- renderer/test/packaged-smoke-entry.test.tsx`; `npm run check`. | Development still enters the ordinary `App`; packaged smoke is gated to `app.isPackaged`, makes zero representative renderer bridge calls, strips smoke controls before the control-plane child, and preserves Electron isolation/IPC boundaries with no migration. | Focused desktop behavior passed 17/17, renderer guard passed 2/2, and the full local regression passed. Existing isolation and enumerated preload IPC settings remain unchanged. | REQ-004 local security/dev-default/zero-bridge/full-regression validation **PASS**. R3 has **no dedicated AC** for REQ-004; exact PR/main CI remains subject to AC-006 and is **NOT VERIFIED**. |
